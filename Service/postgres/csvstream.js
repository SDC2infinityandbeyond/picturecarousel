...
const writeProperties = fs.createWriteStream('./postgres/properties2.csv');
writeProperties.write('propertyid, hostname, hoststatus, city, ussate, country, propertytype, propertyname, guests, bedrooms, beds, baths, propertydescription\n', 'utf8');

const writeImages = fs.createWriteStream('./postgres/images2.csv');
writeImages.write('imageid, propertyId, imagedescription, imageurl\n', 'utf8');

const datasize = 10000000;

let hostStatus = ['Null', 'Superhost'];
let propertyName = ['Ocean Front', 'Beautiful', 'Sunny', 'Magical', 'Renovated', 'Cute', 'Stunning', 'Stylish', 'Amazing', 'Spacious', 'Ocean view', 'Clean', 'Modern', 'Private', 'Dreamy', 'Gorgeous', 'Idyllic', 'A slice of', 'Jewel', 'Cozy', 'Peaceful', '5 star', 'Lovely', 'Charming', 'Newly renovated', 'Sanitized', 'New!'];
let propertyType = ['Apartment', 'Condomonium', 'Loft', 'Serviced apartment', 'House', 'Bungalow', 'Cabin', 'Chalet', 'Cottage', 'Dome house', 'Earth house', 'Farm stay', 'Houseboat', 'Hut', 'Lighthouse', 'Tiny house', 'Townhouse', 'Villa', 'Guest house', 'Guest stay', 'Barn', 'Boat', 'Bus', 'Camper/RV', 'Campsite', 'Castle', 'Cave', 'Hut', 'Igloo', 'Island', 'Lighthouse', 'Plane', 'Tent', 'Tipi', 'Train', 'Treehouse', 'Windmill', 'Yurt', 'Bed and breakfast', 'Nature lodge', 'Boutique hotel', 'Aparthotel', 'Hostel', 'Hotel', 'Resort'];
...
function writeLotsOfProperties(writer, encoding, callback) {
  ...
  function write() {
    ...
    do {
      ...
      const data = `${id},${hostname},${hoststatus},${city},${usstate},${country},${propertytype},${propertyname},${guests},${bedrooms},${beds},${baths},${propertydescription}\n`;
      if (i === 0) {
